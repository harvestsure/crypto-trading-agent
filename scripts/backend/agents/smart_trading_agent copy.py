...
class SmartTradingAgent(BaseAgent):
    ...
    def _build_llm_prompt(
        ...
        """
        构建完整的LLM提示词，包含所有必要的交易上下文
        使用结构化格式确保LLM能够准确理解市场状态并做出合理决策
        """
        # 构建详细的技术分析部分
        ...
        trend_strength = "强趋势" if adx > 25 else "震荡" if adx > 20 else "弱趋势"
        ...
        price_position = "价格在均线上方" if current_price > ema20 else "价格在均线下方"
        ema_trend = "多头排列" if ema20 > ema50 else "空头排列"
        
        # 波动率分析
        atr = indicators.get('ATR', 0)
        current_volatility = "高" if atr / current_price > 0.02 else "中" if atr / current_price > 0.01 else "低"
        
        # MACD分析
        macd = indicators.get('MACD', 0)
        macd_signal = indicators.get('MACD_Signal', 0)
        macd_status = "MACD金叉" if macd > macd_signal else "MACD死叉"
        
        # 构建完整提示词
        prompt = f"""
请基于以下市场数据进行专业的交易分析和决策。

=== 交易标的信息 ===
交易对: {market_context.symbol}
时间周期: {market_context.timeframe}
当前时刻: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

=== 市场行情 ===
当前价格: {current_price:.4f}
24小时成交量: {indicators.get('Volume', 0):.2f}
平均成交量(20周期): {indicators.get('AvgVolume_20', 0):.2f}

最近5根K线:
"""
        # 添加最近5根K线详情
        if market_context.klines:
            recent_klines = market_context.klines[-5:]
            for i, kline in enumerate(recent_klines, 1):
                prompt += f"\n  K线{i}: 开{kline[1]:.4f} 高{kline[2]:.4f} 低{kline[3]:.4f} 收{kline[4]:.4f}"
...
- {macd_status}
  ...
  - Histogram: {indicators.get('MACD_Histogram', 0):.4f}

【波动率指标】
- ATR: {atr:.4f} ({current_volatility}波动)
- 布林带上轨: {indicators.get('BB_Upper', 0):.4f}
- 布林带中轨: {indicators.get('BB_Middle', 0):.4f}
- 布林带下轨: {indicators.get('BB_Lower', 0):.4f}

【KDJ指标】
- K值: {indicators.get('K', 50):.2f}
- D值: {indicators.get('D', 50):.2f}

{position_context.to_llm_prompt()}

{account_context.to_llm_prompt()}
...
2. open_short: 开空单（做空）
3. close_position: 平仓
4. set_stop_loss: 设置止损
5. set_take_profit: 设置止盈
6. get_market_info: 获取更详细的市场信息
7. cancel_orders: 取消挂单

=== 决策要求 ===

请严格按照你的系统提示词中的交易策略和风险管理规则，分析当前市场情况：

1. **市场环境评估**: 判断当前是趋势市还是震荡市，趋势方向和强度
2. **入场条件检查**: 是否满足你的入场标准（技术指标、趋势确认等）
3. **风险评估**: 计算本次交易的风险暴露，是否在可接受范围内
4. **持仓管理**: 如有持仓，评估是否需要止盈、止损或继续持有
5. **具体决策**: 给出明确的操作建议（开多/开空/平仓/持有）

如果决定交易，请使用相应的工具执行。如果持有观望，请说明原因。

记住：保守总比激进好，不确定时选择观望。质量优于数量。
"""
        ...
        if tool_calls:
            ...
            for tool_call in tool_calls:
                ...
                try:
                    ...
                    # 添加工具结果到对话
                    self.llm_model.add_tool_result(tool_id, result_str)
                    ...
                    self.llm_model.add_tool_result(tool_id, f"ERROR: {error_msg}")
            ...
            logger.info("LLM response: No tool calls, observation only")
        
        # 记录token和成本信息
        cache_info = response.get("cache", {})
        logger.info(
            f"Tokens used: {response.get('total_tokens', 0)} "
            f"(Input: {response.get('input_tokens', 0)}, Output: {response.get('output_tokens', 0)}), "
            f"Cost: ${response['cost']['total_cost']:.6f}, "
            f"Cache Hit: {cache_info.get('hit', False)}, "
            f"Tokens Saved: {cache_info.get('read_tokens', 0)}"
        )
